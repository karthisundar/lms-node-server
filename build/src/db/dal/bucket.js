...
exports.deleteBucket = exports.getBucket = exports.getAllBuckets = exports.createBucket = exports.deleteSession = exports.getAllSession = exports.getSessionByRefId = exports.createSession = exports.sessionUser = void 0;
...
const createBucket = async (queryData, payload) => {
    try {
        ...
        // =====================================================
        if (bucketRefId) {
            const existingBucket = await BucketMaster_1.default.findOne({
                where: {
                    bucketRefId,
                },
            });
            ...
            await config_1.default.transaction(async (transaction) => {
                await BucketMaster_1.default.update(updateObj, {
                    where: {
                        bucketRefId,
                    },
                    transaction,
        ...
        // =====================================================
        const newBucketObj = {
            bucketRefId: (0, crypto_1.randomUUID)(),
            bucketName,
            serviceUrl,
            status,
            createdBy: findUser?.user_id,
        ...
        await config_1.default.transaction(async (transaction) => {
            await BucketMaster_1.default.create(newBucketObj, {
                transaction,
            });
        });
        return true;
        ...
        logger_1.default.error("Error createBucket/bucketMaster.ts", error);
        throw error;
    }
};
exports.createBucket = createBucket;
const getAllBuckets = async (queryData) => {
    try {
        const page = Math.max(Number(queryData?.page) || 1, 1);
        const pageSize = Math.max(Number(queryData?.pageSize) || 10, 1);
        // const offset = (page - 1) * pageSize;
        ...
        // GET PAGINATED DATA
        // =====================================================
        const { count, rows } = await BucketMaster_1.default.findAndCountAll({
            where: whereCondition,
            // limit: pageSize,
            // offset,
            nest: true,
        ...
        // await rows?.
        const pagination = await (0, pagination_1.paginationService)(rows, page, pageSize);
        // console.log(pagination, "pagination", rows?.length);
        return pagination;
    }
    catch (error) {
        logger_1.default.error("Error getAllBuckets/bucketMaster.ts", error);
        throw error;
    }
};
exports.getAllBuckets = getAllBuckets;
const getBucket = async (queryData) => {
    const getBucketMaster = await BucketMaster_1.default.findOne({
        where: {
...
const deleteBucket = async (bucketRefId, payload) => {
    try {
        if (!bucketRefId) {
        ...
        await config_1.default.transaction(async (transaction) => {
            await BucketMaster_1.default.destroy({
                where: {
                    bucketRefId,
                },
                transaction,
        ...
        logger_1.default.error("Error deleteBucket/bucketMaster.ts", error);
        throw error;
    }
};
